...
as $$
begin
	return query select id, tag, def from tags.dependency where tag % tag_
		order by tag;
end;
$$ language plpgsql;
...
create or replace function tags.entity_search(
	_tag	text
) returns setof tags.entity
as $$
begin
	return query select id, tag, def from tags.entity where tag % tag_
		order by tag;
end;
$$ language plpgsql;
